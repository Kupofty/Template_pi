...
:: win_deps.bat ends with refreshenv, which rebuilds PATH from the
:: registry and drops the session-only PATH set by vcvars32.bat in
:: appveyor.yml. Restore it here.
set "VS_HOME=C:\Program Files\Microsoft Visual Studio\2022"
call "%VS_HOME%\Community\VC\Auxiliary\Build\vcvars32.bat"

:: Pin the python that win_deps.bat just installed cloudsmith-cli /
:: cryptography into, so refreshenv reordering PATH can't point us
:: at a different interpreter later.
...
%PYTHON_EXE% %SCRIPTDIR%..\ci\git-push
cd ..